...
class Iptv(object):
    ...
    def outPut(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)).replace('python', 'http'), 'tv.m3u'),
                  'w') as f:
            f.write("#EXTM3U\n")
            for item in result:
                className = '其他频道'
    ...
    def outJson(self):
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)).replace('python', 'http'), 'tv.json'),
                  'w') as f:
            f.write(jsonStr)
